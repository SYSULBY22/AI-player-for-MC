...
export class Qwen {
    ...
    constructor(model_name, url, params) {
        ...
        const embedBase = process.env.EMBEDDING_API_BASE || 'https://dashscope.aliyuncs.com/compatible-mode';
        const defaultBase = embedBase.endsWith('/v1')
            ? embedBase
            : `${embedBase.replace(/\/$/, '')}/v1`;
        config.baseURL = url || defaultBase;
        config.apiKey = hasKey('QWEN_API_KEY') ? getKey('QWEN_API_KEY') : getKey('EMBEDDING_API_KEY');
