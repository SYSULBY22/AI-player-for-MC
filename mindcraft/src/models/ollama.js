...
export class Ollama {
    ...
    async sendRequest(turns, systemMessage) {
        ...
        while (attempt < maxAttempts) {
            ...
            // deepseek-r1 等推理模型会输出思考块，剥离后再发到游戏聊天
            const thinkOpen = '<' + 'think' + '>';
            const thinkClose = '<' + '/' + 'think' + '>';
            res = res
                .replace(new RegExp(thinkOpen + '[\\s\\S]*?' + thinkClose, 'gi'), '')
                .replace(/<think>[\s\S]*?<\/redacted_thinking>/gi, '')
                .trim();

            const hasOpenTag = res.includes('<think>') || (res.includes(thinkOpen) && !res.includes(thinkClose));
            const hasCloseTag = res.includes('</think>') || res.includes(thinkClose);

            if ((hasOpenTag && !hasCloseTag)) {
                console.warn("Partial thinking block detected. Re-generating...");
